# Müfredat — AIMA 4. Baskı Yol Haritası

Toplam **28 bölüm**, yaklaşık **28–36 hafta** (haftada 1 bölüm tempo; zor bölümlerde 1.5–2 hafta).

Durum: 🟢 hazır · 🟡 stub / yakında · ⚪ planlı

---

## Bölüm I — Yapay zekâya giriş

**Öğrenme çıktıları:** AI’nin ne olduğunu, rasyonel ajan fikrini, PEAS ve ortam özelliklerini açıklayabilmek; basit ajan mimarilerini ayırt edebilmek.

| # | Klasör | Konu (AIMA TOC) | Hafta | Durum |
|---|--------|-----------------|-------|-------|
| 1 | `ch01-giris/` | Giriş (What is AI?, temeller, tarihçe özeti) | 1 | 🟢 hazır |
| 2 | `ch02-akilli-ajanlar/` | Akıllı ajanlar | 2 | 🟢 hazır |

---

## Bölüm II — Problem çözme

**Öğrenme çıktıları:** Arama problemi formüle etmek; kör ve bilgilendirilmiş aramayı uygulamak; yerel arama, rakip arama (oyunlar) ve kısıt sağlama problemlerini çözmek.

| # | Klasör | Konu | Hafta | Durum |
|---|--------|------|-------|-------|
| 3 | `ch03-cozum-arama/` | Çözüm arama yoluyla problem çözme | 3–4 | 🟢 hazır |
| 4 | `ch04-karmasik-ortamlar/` | Karmaşık ortamlarda arama | 5 | 🟢 hazır |
| 5 | `ch05-rakip-arama/` | Rakip arama ve oyunlar | 6–7 | 🟢 hazır |
| 6 | `ch06-kisit-saglama/` | Kısıt sağlama problemleri | 8 | 🟢 hazır |

---

## Bölüm III — Bilgi, akıl yürütme ve planlama

**Öğrenme çıktıları:** Önermeler mantığı ve birinci mertebe mantıkla bilgi temsil etmek; çıkarım yapmak; klasik planlama algoritmalarını anlamak.

| # | Klasör | Konu | Hafta | Durum |
|---|--------|------|-------|-------|
| 7 | `ch07-mantiksal-ajanlar/` | Mantıksal ajanlar | 9 | 🟢 hazır |
| 8 | `ch08-birinci-derece-mantik/` | Birinci derece mantık | 10 | 🟢 hazır |
| 9 | `ch09-cikarim-birinci-derece/` | Birinci derece mantıkta çıkarım | 11 | 🟢 hazır |
| 10 | `ch10-bilgi-temsili/` | Bilgi temsili | 12 | 🟢 hazır |
| 11 | `ch11-klasik-planlama/` | Klasik planlama | 13 | 🟢 hazır |

---

## Bölüm IV — Belirsiz bilgi ve akıl yürütme

**Öğrenme çıktıları:** Olasılık ve Bayes ağlarıyla belirsizliği modellemek; zamansal modeller, karar teorisi ve Markov karar süreçlerini kullanmak.

| # | Klasör | Konu | Hafta | Durum |
|---|--------|------|-------|-------|
| 12 | `ch12-belirsiz-bilgi/` | Belirsiz bilginin nicelendirilmesi | 14 | 🟢 hazır |
| 13 | `ch13-olasiliksal-akil/` | Olasılıksal akıl yürütme | 15–16 | 🟢 hazır |
| 14 | `ch14-zamansal-olasilik/` | Zamansal olasılıksal modeller | 17 | 🟢 hazır |
| 15 | `ch15-olasiliksal-programlama/` | Olasılıksal programlama | 18 | 🟢 hazır |
| 16 | `ch16-basit-kararlar/` | Basit kararlar alma | 19 | ⚪ planlı |
| 17 | `ch17-karmasik-kararlar/` | Karmaşık kararlar alma | 20 | ⚪ planlı |
| 18 | `ch18-cok-ajanli-karar/` | Çok ajanlı karar verme | 21 | ⚪ planlı |

---

## Bölüm V — Makine öğrenmesi

**Öğrenme çıktıları:** Denetimli öğrenme, öğrenme teorisi, derin öğrenme, pekiştirmeli öğrenme ve olasılıksal modellerle öğrenmeyi uygulamak.

| # | Klasör | Konu | Hafta | Durum |
|---|--------|------|-------|-------|
| 19 | `ch19-ogrenme-orneklerden/` | Örneklerden öğrenme | 22–23 | ⚪ planlı |
| 20 | `ch20-bilgi-ogrenme/` | Bilgi öğrenme | 24 | ⚪ planlı |
| 21 | `ch21-derin-ogrenme/` | Derin öğrenme | 25 | ⚪ planlı |
| 22 | `ch22-pekistirmeli-ogrenme/` | Pekiştirmeli öğrenme | 26 | ⚪ planlı |

---

## Bölüm VI — İletişim, algılama ve eylem

**Öğrenme çıktıları:** Doğal dil, bilgisayarlı görü ve robotik konularının temel problemlerini ve yaklaşımlarını özetlemek.

| # | Klasör | Konu | Hafta | Durum |
|---|--------|------|-------|-------|
| 23 | `ch23-dogal-dil/` | Doğal dil işleme | 27 | ⚪ planlı |
| 24 | `ch24-derin-dil/` | Derin öğrenme ile doğal dil | 28 | ⚪ planlı |
| 25 | `ch25-bilgisayarli-goru/` | Bilgisayarlı görü | 29 | ⚪ planlı |
| 26 | `ch26-robotik/` | Robotik | 30 | ⚪ planlı |

---

## Bölüm VII — Sonuçlar

**Öğrenme çıktıları:** AI’nin felsefi ve etik boyutlarını tartışmak; geleceğe dair bilinçli bir bakış geliştirmek.

| # | Klasör | Konu | Hafta | Durum |
|---|--------|------|-------|-------|
| 27 | `ch27-felsefe-etik/` | Felsefe, etik ve AI güvenliği | 31 | ⚪ planlı |
| 28 | `ch28-AI-gelecek/` | AI’nin geleceği | 32 | ⚪ planlı |

---

## Tempo önerisi

- **Hızlı:** haftada 1 bölüm (~7–8 ay)
- **Rahat:** zor bölümlerde 1.5–2 hafta (~9 ay)
- Her bölüm sonunda: not → kod → alıştırma → quiz döngüsünü tamamlayın

## Resmi kaynaklar

- [AIMA resmi site](https://aima.cs.berkeley.edu/)
- [aimacode (GitHub)](https://github.com/aimacode)
